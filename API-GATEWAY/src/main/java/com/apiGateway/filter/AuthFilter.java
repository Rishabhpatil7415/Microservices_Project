...
public class AuthFilter extends AbstractGatewayFilterFactory<AuthFilter.Config> {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
        	//true when this not out of 3 endpoints
            if(validator.predicate.test(exchange.getRequest())) {
            	
            	if (exchange.getRequest().getHeaders().getFirst(HttpHeaders.AUTHORIZATION) == null) {  //throw exception when header missing
                	//this will check header is present or not
